...
public class ListenerInternal implements Listener {
    ...
    @EventHandler
    public void e(UranusScriptEvalEvent e) {
        if (e.getProgram() instanceof QuestProgram) {
            e.getBindings().put("event", ((QuestProgram) e.getProgram()).getEvent());
            e.getBindings().put("e", ((QuestProgram) e.getProgram()).getEvent());
        }
    }
