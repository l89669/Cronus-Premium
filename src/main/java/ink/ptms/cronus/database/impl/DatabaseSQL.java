...
public class DatabaseSQL extends Database {
    ...
    public void init() {
        ...
        table = new SQLTable(Cronus.getConf().getStringColored("Database.table"), SQLColumn.PRIMARY_KEY_ID, new SQLColumn(SQLColumnType.VARCHAR, 36, "player"), new SQLColumn(SQLColumnType.LONGTEXT, "data"));
        tableVar = new SQLTable(Cronus.getConf().getStringColored("Database.table") + "_variable", SQLColumn.PRIMARY_KEY_ID, new SQLColumn(SQLColumnType.VARCHAR, 128, "key"), new SQLColumn(SQLColumnType.LONGTEXT, "value"));
        try {
